...
class SeedsBuilder:
    ...
    def __init__(
        ...
        operations_gateway_client: (
            OperationsGatewayGRPCClient | OperationsGatewayHTTPClient
        ),
    ):
    ...
    def build_virtual_card_result(
        ...
        """
        Выпускает dbhnefkmye. карту для заданного пользователя и счёта.
    ...
    def build_transfer_operation_result(
        ...
        """
        Выполняет операцию перевода с карты.
    ...
    def build_cash_withdrawal_operation_result(
        ...
        """
        Выполняет операцию снятия наличных с карты.
    ...
    def build_debit_card_account_result(
        ...
        return SeedAccountResult(
            ...
            virtual_cards=[
                self.build_virtual_card_result(
                    user_id=user_id, account_id=response.account.id
                )
